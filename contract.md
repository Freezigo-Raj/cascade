...
| `chip_presets` | the date chips on screen 2, one per phrase the engine reads, each proved against the engine before joining. Screen vocabulary: a tapped chip's words join the line the engine reads (under a `chip_span`), without appearing in the box — the box holds what the person typed and the tick chip shows the reading. The two pickers are screen furniture, pinned beside the tick, not members of this list. | — |
